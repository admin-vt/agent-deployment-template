...
- **Vendor incident note:** Composio Remote Sandbox degraded on 2026-07-20 (~01:30 UTC onward): first full transport timeouts, then — diagnosed via in-sandbox probe — **outbound network to GitHub dead** (`curl -sI https://github.com` returns nothing; same probe returned HTTP 200 hours earlier). Fast local commands work; anything needing egress hangs. Catalog tools (Firecrawl) unaffected throughout. The agent (Sonnet 4.5) surfaced the outage gracefully to the user rather than failing silently.
- **Pending (vendor-blocked, ~2-min check):** the re-prove deployment's workspace git-loop demo. The identical code path is fully proven on vt-poc (commit `first workspace task` + file on GitHub). Re-run when sandbox egress recovers: ask the proof agent to `workspace_init` → write → `workspace_commit`, then confirm the commit on `admin-vt/template-proof-agent-workspace`.
