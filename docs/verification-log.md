...
- **OPEN — workspace loop not yet proven end-to-end on a deployed agent.** Through the deployed agent, a `workspace_run` failed with `cd: /mnt/files/workspace: No such file or directory` immediately after its own preamble should have cloned — with no clone/mktemp errors in stderr. Suspicion: the sandbox backend may not execute the full compound command (splitting/truncation), or mounts detach mid-session. Next diagnostic: run the same command with `echo SEG1..SEG5` markers between segments to see which parts execute. If compound commands are unreliable, the fix is writing a script file and executing it (or Composio's local sandbox mode) instead of multi-statement strings. **The workspace loop must be re-verified before any client relies on it.**
