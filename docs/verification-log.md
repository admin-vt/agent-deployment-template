...
## ari-clickup deployment (first true runbook clone, 2026-07-20) — findings

The first post-de-instantiation runbook run (repo `Viral-Tilt/ari-clickup-agent`, agent "Otto") reached: agent live and answering, memory round-trip verified, onboarding deployed and gated, Slack app created via the script. It also surfaced the following, all fixed in the template except where noted:

- **Doppler multi-set syntax:** `doppler secrets set NAME1 value1 NAME2 value2` fails — multi-set requires `NAME=VALUE` form. Runbook corrected.
- **Onboarding Vercel deploy needs rootDirectory=onboarding — VERIFIED.** `cd onboarding && vercel deploy` uploads only that directory, so the deliberate repo-root import (`slack-bot-scopes.json`) breaks the build. Fix: set the project's Root Directory to `onboarding` (`PATCH /v9/projects/<id>?teamId=<team>` — no CLI flag in 54.x), copy the `.vercel/` link to the repo root, deploy from the repo root. Verified working; runbook §5 rewritten.
- **Endpoint timing:** the Mastra endpoints sit behind Cloudflare, which 524s at ~100s; a workspace tool call on a cold sandbox takes ~2 min (session + sandbox provisioning + clone). Verification must use `/stream` for tool-heavy turns; runbook §6 rewritten.
- **Duplicate dispatch re-verified live, and it broke the original workspace tools:** a single bash execute was dispatched twice concurrently. The V4-era `[ -d .git ] || git clone` is check-then-act and races with itself; worse, one dispatch's self-heal (`rev-parse || rm -rf`) can destroy the repo the other dispatch just cloned — *after* success was already reported. `src/lib/workspace.ts` hardened (developed and race-tested 4-way on ari-clickup, ported to template): mkdir-mutex with 5-min stale-lock stealing serializes repo repair; clone lands via clone-to-temp + atomic `mv -n -T`; `WORKSPACE_READY` only echoes after a fresh `rev-parse` passes; the ensure-repo preamble runs before every tool so run/commit self-heal too.
- **Mount semantics verified:** `/mnt/files` is per Composio *session* (marker file persisted within a session; a second session sees an empty mount). Multiple Mastra server instances each hold their own cached session → divergent sandboxes. Tolerable because the workspace repo is the only durable state and every tool call now self-heals, but out-of-band sandbox experiments cannot touch the agent's own mount.
- ~~OPEN — workspace loop mystery~~ **ROOT-CAUSED AND FIXED (2026-07-20, see next entry):** `/mnt/files` is an s3fs mount; git must not live there. The compound-command-splitting hypothesis was disproven.
- **Mastra deploy warning is misleading:** every deploy logs `No storage configured — falling back to in-memory store... all data is lost on restart`, yet the platform-side memory round-trip verifies fine (fact stored in one request, recalled in a second). Treat the warning as benign on platform deploys; the platform provisions hosted storage regardless (consistent with V1).

## Workspace-loop failure root-caused: git-on-s3fs — FIXED & RE-PROVEN (2026-07-20)

Diagnosed by driving `COMPOSIO_REMOTE_BASH_TOOL` directly (three instrumented rounds, then an e2e harness driving the real tool implementations).

**Disproven:** compound-command splitting/truncation. All separators (`;`, `&&`, `||`), `for`/`$(...)`/subshells, 8k-char commands, and stderr capture execute fully and correctly.

**Established (each observed directly):**
- `/mnt/files` is **s3fs** (FUSE over S3; sandboxes are E2B microVMs, `hostname` = `e2b.local`). s3fs has no atomic rename (`mv -T` = copy+delete, ~4s for a tiny dir), `rm -rf` fails on non-empty trees (`cannot remove '.git/refs': Directory not empty`), and directory listings go stale between calls. git — built on atomic renames and lock files — corrupts there under concurrency. Clone comparison: **78s on s3fs vs 1.6s on local ext4**.
- The exact deployed failure (`cd: <dir>: No such file or directory` immediately after a clone, clean stderr) was **reproduced on demand** with git-on-s3fs + duplicate dispatch.
- A command whose SDK request times out (~181s client-side) **keeps running server-side** (probe process still alive 90s after the client threw) — abandoned commands silently mutate state while retries run: invisible concurrency on top of the known duplicate dispatch.
- Concurrent executes within one session share one sandbox and filesystem; `sandbox_id_suffix` stable across calls. Local layout: `/home/user` on ext4 (~20GB free); `/tmp` is a 500MB tmpfs (too small).

**Fix (`src/lib/workspace.ts`):** workspace moved to sandbox-local disk (`/home/user/workspace`) — the repo push is the durability mechanism, so the s3fs mount's per-session persistence buys nothing; mkdir-mutex now actually atomic (local fs) with `trap`-based release and 5-min stale-steal; network git ops bounded with `timeout` so abandoned commands can't mutate state for long; on lock exhaustion repair is skipped (never repair lock-less) and a readiness guard makes every tool answer `WORKSPACE_NOT_READY … retry` instead of acting on a broken tree.

**Re-proven e2e** against a scratch repo through the real tool implementations: 4 concurrent `workspace_init` all `WORKSPACE_READY`; run → commit → push landed on GitHub (`ws-fix e2e proof`); deliberate `.git` destruction self-healed on the next init; mixed concurrent init/run/commit clean.

## Stale-bundle gotcha + workspace loop VERIFIED LIVE (2026-07-20)

- **Vendor gotcha (verified): `mastra deploy` re-ships an existing `.mastra/output` bundle without rebuilding changed sources.** Symptom: "Deploy succeeded in ~47s" with none of your changes live — two successive workspace fixes on ari-clickup "deployed" this way while the container kept running the original code (a genuine fresh build takes ~15+ min). `scripts/deploy.sh` now removes `.mastra` before every deploy. Also observed live during diagnosis: the dead s3fs mount in the old cached sandbox surfaced as `cd: /mnt/files/workspace: Transport endpoint is not connected` — s3fs FUSE crashes mid-session, further confirming the move off s3fs.
- **Workspace loop verified end-to-end through the deployed agent** (ari-clickup, fresh build with local-disk workspace): `workspace_init` → `WORKSPACE_READY` with repo contents; `workspace_run` wrote `verified.txt`; `workspace_commit` pushed — commit `4303b05 "workspace loop verified"` and file content confirmed on GitHub. The last open gate on the workspace layer is closed.
- Bonus verification: on boot without a Slack install, the adapter's `auth.test` fails soft exactly as designed (`No Slack bot token: app not installed to a workspace yet`, warn-only, boot continues).

## vt-twitter deployment findings: not all OAuth toolkits are Composio-managed; auth configs are a deploy prerequisite (2026-07-21)

Harvested from the vt-twitter-agent stand-up (second true runbook clone); both upstreamed into runbook §3 and docs/customization.md.

- **Composio has no managed OAuth credentials for the `twitter` toolkit — VERIFIED.** `authConfigs.create('twitter', { type: 'use_composio_managed_auth' })` fails with code 306 `Auth_Config_DefaultAuthConfigNotFound`. The toolkit's only auth scheme is OAUTH2 with **required `client_id`/`client_secret`** — every twitter deployment needs its own X developer app (developer.twitter.com; browser signup, possibly paid tier). The "OAuth toolkits use managed auth" pattern doesn't hold universally: check `toolkits.get(<slug>).authConfigDetails` before assuming, and fall back to `{ type: 'use_custom_auth', authScheme: 'OAUTH2' }` with client-owned app credentials.
- **A listed toolkit without an auth config breaks the agent at session creation — VERIFIED (correcting the earlier assumption that it was safe).** With `toolkits: ['twitter']` and no auth config in the project, every `/generate` returns `400 ToolRouterV2_BadRequest code 4300: "toolkits require auth configs but none exist and cannot be auto-created: twitter"` — `composio.create()` requires an auth config for every bound toolkit; binding only ACTIVE connections (`src/lib/composio.ts`) does not exempt the toolkit list itself. Rule: create each toolkit's auth config **before** deploying with that toolkit listed, or deploy with `toolkits: []` and redeploy once the config exists.
