...
| D3 | Model billing | **BYOK.** Each client sets their own OpenRouter key on the deployment's agent account (D12); the agent's model calls bill the client's balance. |
| D4 | Tools | **Composio, used natively and fully.** All agent tools come through Composio's catalog and SDK — tool auth (the credential holder authorizes accounts for the agent, D12), tool execution, and triggers. The full native capability is used directly. |
...
3. **OpenRouter BYOK wiring** — the client's key lives on the agent account; the agent's model calls bill the client's OpenRouter account.
4. **A Composio project** — the agent's entire tool layer: the tools deliberately authorized for this agent (connected accounts on the agent account, in this deployment's own Composio project), executing through Composio's catalog.
...
6. **A WorkOS-authenticated onboarding surface (agent console)** — where the credential holder signs in as the agent account, authorizes the agent's tools (Composio auth links), sets the client's OpenRouter key, adds the agent to Slack, and manages the allowlist. Hosted on Vercel.

---

## 4. V1 scope

1. **The PoC agent, live**: agent on Mastra Hosted — Composio Firecrawl tool wired, one web-research `SKILL.md`, answering in Slack, WorkOS-gated onboarding page, sandbox + workspace-repo pattern working, model calls billing to the key set on the agent account (client BYOK).
...
1. **PoC live**: a Slack message to the agent returns a real web-search-backed answer via the Composio Firecrawl tool; the model call bills the OpenRouter balance of the key on the agent account; the agent completes one sandbox task whose result lands in its workspace repo.
...
3. **BYOK is verified, not assumed.** A two-account test (operator key vs. client key) confirms each call bills its own key's owner before any client relies on it.
